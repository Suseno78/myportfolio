...
export default function LightPillar({
  ...
  useEffect(() => {
    ...
    const rSpeed = rotationSpeed;
    const animate = () => {
      if (!materialRef.current || !rendererRef.current || !sceneRef.current || !cameraRef.current) return;
      timeRef.current += 0.016 * rSpeed;
  ...
  }, [webGLSupported, quality, topColor, bottomColor, intensity, rotationSpeed, glowAmount, pillarWidth, pillarHeight, noiseIntensity]);

  if (!webGLSupported) return null;

  return <div ref={containerRef} className={`w-full h-full ${className}`} style={{ mixBlendMode: 'screen' }} />;
}
