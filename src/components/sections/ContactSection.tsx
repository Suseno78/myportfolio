...
const coloredDockIcons: Record<string, React.ReactNode> = {
  email: (
...
export default function ContactSection() {
  const dockItems = socialLinks.map((link) => ({
    icon: coloredDockIcons[link.icon] || <span>📧</span>,
    label: link.name,
    ...
    <section id="contact" className="relative py-28 md:py-36 px-6">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <ScrollReveal delay={0.1}>
          ...
          <p className="text-zinc-600 dark:text-zinc-500 text-lg mb-16 max-w-md mx-auto">
            Feel free to reach out or follow me on social media.
          </p>
        </ScrollReveal>

        {/* Dock container with glow border */}
        <ScrollReveal delay={0.3}>
          <div className="relative inline-block">
            {/* Glow behind dock */}
            <div className="absolute -inset-4 rounded-3xl bg-gradient-to-r from-violet-500/10 via-cyan-500/10 to-violet-500/10 dark:from-violet-500/20 dark:via-cyan-500/20 dark:to-violet-500/20 blur-2xl opacity-60 footer-glow-anim" />

            {/* Glassmorphism container */}
            <div className="relative rounded-2xl border border-zinc-200/80 dark:border-zinc-700/50 bg-white/60 dark:bg-zinc-900/40 backdrop-blur-2xl shadow-2xl shadow-violet-500/5 p-6 md:p-8">
        ...
        <ScrollReveal delay={0.5}>
          <div className="mt-12 flex items-center justify-center gap-2 text-zinc-500 dark:text-zinc-600 text-sm">
